...
using namespace std; CGlaive::CGlaive(void)
	: CProjectile(NULL)
	, m_fLength(0.0)
	, angle_x(0.0)
	, angle_y(0.0)
...
CGlaive::CGlaive(Mesh* _modelMesh)
...
void CGlaive::SetLength(const float m_fLength)
{
	this->m_fLength = m_fLength;
}

// Get the length of the laser
float CGlaive::GetLength(void) const
{
	return m_fLength;
}

// Update this laser
void CGlaive::CalculateAngles(void)
{
...
CGlaive* Create::Glaive(const std::string& _meshName, const Vector3& _position, const Vector3& _direction, const float m_fLength, 
	const float m_fLifetime, const float m_fSpeed, CPlayerInfo* _source)
{
	...
	CGlaive* result = new CGlaive(modelMesh);
