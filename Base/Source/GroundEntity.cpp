...
GroundEntity::GroundEntity(Mesh* _modelMesh)
: position(0.0f, 0.0f, 0.0f)
, scale(1.0f, 1.0f, 1.0f)
, size(1.0f, 1.0f, 1.0f)
, grids(9.0f, 0.0f, 9.0f)
, maxBoundary(1.0f, 1.0f, 1.0f)
, minBoundary(1.0f, 1.0f, 1.0f)
, m_bMaxBoundaryDefined(false)
, m_bMinBoundaryDefined(false)
{
	modelMesh[0] = _modelMesh;
	modelMesh[1] = nullptr;
}
...
void GroundEntity::Render()
{
	...
	for (int x = 0; x < (int)grids.x; x++)
	{
		for (int z = 0; z < (int)grids.z; z++)
		{
			...
			//	else
			//if (CSpatialPartition::GetInstance()->GetGrid(2, 2).GetListOfObject().empty())
			float sizeX = scale.x * grids.x;
			float sizeZ = scale.z * grids.z;
			float indexX = sizeX * -0.5f + scale.x * x;
			float indexZ = sizeZ * -0.5f + scale.z * z;
			if (CSpatialPartition::GetInstance()->GetObjects(Vector3(indexZ, 0, indexX), 0).empty())
				RenderHelper::RenderMesh(modelMesh[0]);
			else
				RenderHelper::RenderMesh(modelMesh[1]);
...
GroundEntity* Create::Ground(const std::string& _meshName1)
{
	Mesh* modelMesh1 = MeshBuilder::GetInstance()->GetMesh(_meshName1);
	if (modelMesh1 == nullptr)
		return nullptr;

	GroundEntity* result = new GroundEntity(modelMesh1);
	EntityManager::GetInstance()->AddEntity(result);
	return result;
}
