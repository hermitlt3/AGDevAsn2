...
CSceneNode* CSceneNode::AddChild(EntityBase* theEntity)
{
	if (theEntity)
	{
		...
		Mtx44 temp;
		temp.SetToScale(theEntity->GetScale().x, theEntity->GetScale().y, theEntity->GetScale().z);
		aNewNode->GetTransform() = temp * aNewNode->GetTransform(); 
		temp.SetToIdentity();
		temp.SetToTranslation(theEntity->GetPosition().x, theEntity->GetPosition().y, theEntity->GetPosition().z);
		aNewNode->GetTransform() = temp * aNewNode->GetTransform();
		// Add to vector list
...
CTransform CSceneNode::GetNodeLocalTransform()
{
	CSceneNode* curr = this;
	CTransform results;

	// Transverse backwards from node to root
	while (curr != CSceneGraph::GetInstance()->GetRoot()) {
		results.GetTransform() = curr->GetTransform() * results.GetTransform();
		curr = curr->GetParent();
	}
	return results;
}