...
{
	if (CSceneGraph::GetInstance()->GetNode(theObject))
	{
		Vector3 a;
		CSceneGraph::GetInstance()->GetNode(theObject)->GetNodeLocalTransform().GetTranslate(a.x, a.y, a.z);
		int xIndex = (((int)a.x - (-xSize >> 1)) / (xSize / xNumOfGrid));
		int zIndex = (((int)a.z - (-zSize >> 1)) / (zSize / zNumOfGrid));
