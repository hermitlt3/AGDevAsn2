	...
	for (unsigned z = 0; z < terrainSize; ++z) {
		for (unsigned x = 0; x < terrainSize; ++x) {
			...
			if (z > 1 && z < terrainSize - 1 && x > 1 && x < terrainSize - 1) {
				float hL = heightMap[z * terrainSize + x - 1] / SCALE_FACTOR;
				float hR = heightMap[z * terrainSize + x + 1] / SCALE_FACTOR;
				float hU = heightMap[(z - 1) * terrainSize + x] / SCALE_FACTOR;
				float hD = heightMap[(z + 1) * terrainSize + x] / SCALE_FACTOR;
				v.normal = Vector3(hL - hR, 2.0f, hU - hD).Normalize();
			}
			vertex_buffer_data.push_back(v);
		}
	}
...
Mesh* MeshBuilder::GenerateBar(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;

	v.pos.Set(-0.f * length, -0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(1.f * length, -0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f, 0);
	vertex_buffer_data.push_back(v);
